    ...
    var game_variables = {
        total_words: 0, // add in case where the user deletes the word and there is no other words hint: do an if statement
        current_word_id: 0,
        //:;
        words_created: 0,
    ...
    function life_decrease() {
        // by checking if any of the current elements are below window size and decrease life
        // will need to perform loop
        // also responsible for wiping screen when life reaches zero
    }
    ...
    // search function might not be needed set a variable to equal the current_position's innerText || textContent
    function search_tag() {
        for (var i = 0; i <= 2; i++) {
            var temp = i.toString();
            var derp = document.getElementById(temp).innerHTML;
            console.log(derp);
            //var ch = derp.charAt(0);
            //alert(typeof derp) // displays "string"
            //var ch = derp.charAt(0);
            //console.log(ch);
        }

        var here = document.getElementById("0");
        var here1 = here.innerText || here.textContent
        console.log(here1[0]);
        document.getElementById("3").innerHTML = here1[0];
    }

    function New_word() {
        var left_position_int = rand_position();
        //var left_position_string = left_position_int.toString();
        var temp = document.createElement('span');
        temp.id = game_variables.words_created.toString();
        tempSauce = shuffledArray[game_variables.arrayCounter]
        console.log(tempSauce);
        temp.innerHTML = tempSauce;
        if (wordsSoFar.length > wordsMax ) {
        	wordsSoFar.shift();
        	wordsSoFar.push(tempSauce);
        } else {
        	wordsSoFar.push(tempSauce);
        }
        // console.log(wordsSoFar.length);
    ...
    if (answer == "yes") {
        ...
        document.onkeypress = function(event) {
            ...
            if (key_code == 13) {
	            ...
	            var key_character = String.fromCharCode(key_code);
	            output += key_character;
	            check++;
	            if (output != correct && check >= 3) {
	            	var start = game_variables.arrayCounter;
	            	for (var i = wordsSoFar.length; i > 0; i--) {
	            		if (output == wordsSoFar[i-1]){
	            			check = 0;
	            			output = "";
	            			downSpeed *= 0.5;
